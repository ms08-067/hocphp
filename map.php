...
marker.addListener('click', function() {
  ...
  geocoder.geocode({ 'latLng': latlng }, function (results2, status) {
    if (status == google.maps.GeocoderStatus.OK) {
        if (results2[0]) {
            infowindow.setContent("<span id='address'>" + results2[0].formatted_address + "</span>");
            addressReturn = results2[0].formatted_address;
            infowindow.open(map, marker);
        }
    } else {
        alert("Không tìm thấy vị trí trên bản đồ : " + status);
    }
  });
  map.setCenter(point);
});
            ...
            $.post("1.php",{
                ...
                des_map_lng: map_lng
                  ...
                  if(data.rows.length > 0 && data.rows.elements.length > 0){
                      ...
                      var content = "<div style='min-width:200px;line-height: 20px;overflow: auto;' class='infoWindow'><img style='float: left; margin-right: 6px;' width='40' src='<?php echo $avatar ?>'/><h4>Tài Xế: Nguyen Van A</h4><p class='address'>Cách gậy "+ distance +" km</p></div>";
